...
export const AdminShippingPolicyEditor = () => {
  ...
  const fetchPolicy = async () => {
    ...
    setError(null);
    try {
      const res = await api('/api/shipping-policy');
      if (res.ok && res.json?.data) {
        setPolicy(res.json.data as ShippingPolicyData);
      } else if (res.ok && !res.json?.data) {
        setPolicy({ sections: [] }); // No policy found, initialize with empty sections
      } else {
        setError(res.json?.message || 'Failed to fetch shipping policy');
        toast.error(res.json?.message || 'Failed to fetch shipping policy');
      }
    } catch (err) {
  ...
  const handleSavePolicy = async () => {
    ...
    try {
      ...
      if (res.ok && res.json?.data) {
        ...
        setError(res.json?.message || 'Failed to save shipping policy');
        toast.error(res.json?.message || 'Failed to save shipping policy');
      }
    } catch (err) {
      console.error('Error saving shipping policy:', err);
      setError('Network error or server unavailable');
      toast.error('Network error or server unavailable');
    } finally {
      setSaving(false);
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <h1 className="text-3xl font-bold">Edit Shipping Policy</h1>
